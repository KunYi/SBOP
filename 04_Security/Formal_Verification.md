...
LOAD_IMAGE → (PARSE_HEADER | FAILSAFE)
PARSE_HEADER → (OTA_DECRYPT | VERIFY_SIGNATURE | FAILSAFE)
OTA_DECRYPT → (VERIFY_SIGNATURE | FAILSAFE)
VERIFY_SIGNATURE → (VERIFY_INTEGRITY | FAILSAFE)
VERIFY_INTEGRITY → (CHECK_VERSION | FAILSAFE)
CHECK_VERSION → (COMMIT_VERSION | FAILSAFE)
COMMIT_VERSION → (MARK_TESTING | FAILSAFE)
MARK_TESTING → (LOCK_BOOT | FAILSAFE)
LOCK_BOOT → EXECUTE
```
...
| --- | --- | --- |
| state | {RESET, INIT, SELECT_SLOT, LOAD_IMAGE, PARSE_HEADER, OTA_DECRYPT, VERIFY_SIGNATURE, VERIFY_INTEGRITY, CHECK_VERSION, COMMIT_VERSION, MARK_TESTING, LOCK_BOOT, EXECUTE, FAILSAFE, TAMPER_LOCK} | 15 |
